...
class ChatAdapter(val items:List<Message>, val userId: String):RecyclerView.Adapter<RecyclerView.ViewHolder>(){
    ...
    class ViewHolderR(itemView: View):RecyclerView.ViewHolder(itemView){
        fun bind(message:Message) = with(itemView){
          ...
          //  val logo = context.resources.getIdentifier("ic_person", "drawable", context.packageName)

            //Picasso.get().load(R.drawable.ic_person).into(imageViewProfileRight)
            //Picasso load image here
            if(message.profileImageURL.isNullOrEmpty()){
                Log.d(message.profileImageURL.toString()," foto")
                Glide.with(this).load(R.drawable.ic_person).apply(RequestOptions().override(100, 100))
                    .centerCrop().circleCrop().into(imageViewProfileRight)
                /*
    ...
    class ViewHolderL(itemView: View):RecyclerView.ViewHolder(itemView){
        fun bind(message:Message) = with(itemView){
            ...
            //Picasso load image here
            if(message.profileImageURL.isNullOrEmpty()){
                Glide.with(this).load(R.drawable.ic_person).apply(RequestOptions().override(100, 100)).centerCrop().circleCrop().into(imageViewProfileLeft)
                /*
                Picasso.get().load(R.drawable.ic_person).resize(100, 100)
